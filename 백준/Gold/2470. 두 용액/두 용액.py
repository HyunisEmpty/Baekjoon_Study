...
while pos_index != len(pos_list) and len(neg_list) != 0:    # len(neg_list) != 0 neg_list의 길이가 0인 경우에도 무한 반복문이 실행되 실행문 첫번째 neg_list의 원소를 호출하는 IndexError가 발생할 수 있다.
    ...
    else: # 이전 local_abs_min보다 값이 증가하거나 같아진 경우.

        if local_abs_min < n_sum:   # 같아진 경우에는 neg_index가 증가하지 않았으므로 neg_index -= 1을 할 필요가 없지만, neg_index가 증가한 경우 이전 이전 인덱스가 최소값을 가지므로 1을 빼야한다.
            neg_index -= 1
