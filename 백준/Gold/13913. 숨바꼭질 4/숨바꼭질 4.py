...
bfs_deque.append((start_position, second, route))

while len(bfs_deque) != 0:

    position, second, route = bfs_deque.popleft()
    ...
    if position != end_position:
        if position + 1 not in bfs_set and 0 <= position + 1 <= 100000:
            bfs_deque.append((position + 1, second + 1, route + " " + str(position + 1)))
            bfs_set.add(position + 1)
        if position - 1 not in bfs_set and 0 <= position - 1 <= 100000:
            bfs_deque.append((position - 1, second + 1, route + " " + str(position - 1)))
            bfs_set.add(position - 1)
        if position * 2 not in bfs_set and 0 <= position * 2 <= 100000:
            bfs_deque.append((position * 2, second + 1, route + " " + str(position * 2)))
